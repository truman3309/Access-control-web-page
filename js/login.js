// === 🟢 登入驗證功能 ===
function handleLogin(event) {
  event.preventDefault();

  const username = document.getElementById("username").value.trim();
  const password = document.getElementById("password").value.trim();
  const msg = document.getElementById("loginMessage");

  // 重置提示樣式
  msg.className = "message";

  if (!username || !password) {
    msg.textContent = "請輸入完整的登入資訊。";
    msg.classList.add("error");
    return false;
  }

  // === 模擬登入驗證（未串接後端時可用） ===
  if (username === "admin" && password === "1234") {
    msg.textContent = "登入成功，正在跳轉...";
    msg.classList.add("success");

    // 取得目前時間並格式化
    const now = new Date();
    const formattedTime = now.toLocaleString("zh-TW", {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
      hour: "2-digit",
      minute: "2-digit",
    });

    // 儲存登入資料
    localStorage.setItem("userName", username);
    localStorage.setItem("lastLogin", formattedTime);

    // 延遲跳轉
    setTimeout(() => {
      window.location.href = "../html/主頁.html";
    });
  } else {
    msg.textContent = "帳號或密碼錯誤，請重新輸入。";
    msg.classList.add("error");
  }

  return false;
}

// === 🟡 登入狀態檢查與顯示名稱（主頁共用） ===
document.addEventListener("DOMContentLoaded", () => {
  const loginLink = document.querySelector(".login-link"); // 導覽列登入按鈕
  const nameDisplay = document.getElementById("userNameDisplay"); // 顯示使用者名稱
  const userName = localStorage.getItem("userName"); // 取得登入使用者名稱

  if (userName) {
    // ✅ 已登入：顯示名稱、隱藏登入按鈕
    if (loginLink) loginLink.style.display = "none";

    if (nameDisplay) {
      nameDisplay.textContent = userName;
      nameDisplay.style.display = "inline";
      nameDisplay.style.color = "#ffdf5d";
      nameDisplay.style.fontWeight = "bold";
      nameDisplay.style.cursor = "pointer";
      nameDisplay.title = "查看個人基本資料";

      // 🟢 第一次點擊：進入個人基本資料
      // 🟡 第二次（3 秒內）點擊：詢問是否登出
      let firstClick = true;
      nameDisplay.addEventListener("click", () => {
        if (firstClick) {
          window.location.href = "個人基本資料.html";
          firstClick = false;
          setTimeout(() => (firstClick = true), 3000);
        } else {
          const confirmLogout = confirm(`是否要登出帳號「${userName}」？`);
          if (confirmLogout) {
            localStorage.removeItem("userName");
            localStorage.removeItem("lastLogin");
            alert("您已成功登出！");
            window.location.href = "登入.html";
          }
        }
      });
    }
  } else {
    // ❌ 未登入：顯示登入按鈕、隱藏名稱
    if (loginLink) loginLink.style.display = "inline";
    if (nameDisplay) nameDisplay.style.display = "none";
  }
});
